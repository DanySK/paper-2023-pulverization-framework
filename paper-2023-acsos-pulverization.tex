...
basewidth=0.55em % decreases spacing between chars
}
...
\emph{runtime configuration rules}, allowing pulverised components to move at runtime across different hosts;
 \item we accordingly provide a practical implementation of pulverisation (with reconfiguration rules) called \emph{\ourframework{}}, developed in the Kotlin programming language.
\end{enumerate}
%
To evaluate \ourframework{}, we exercise it through two case studies:
a small-scale real-world demonstration, and a city-scale simulated scenario.
...
\caption{
    ...
    and properly configured to interact with other deployment units according to the protocal 
    that the application at hand requires (see  \Cref{sec:evaluation} for examples).
}
\label{fig:approach}
\end{figure}
...
but it has a high maintenance cost
...
\begin{lstlisting}[
    ...
    caption={
        ...
        whose components are bound to the set of capabilities they need to execute.
    },
    label=lst:system-configuration,
    escapechar=\%,
    numbers=left
...
// Runtime initial setup and runtime reconfig. rules %\label{code:infra:start}%
...
%
The framework has been open-sourced and released with a permissive licence\footnote{\url{https://github.com/pulvreakt/pulvreakt}}.
...
%
At the start of the system, using the provided configuration, the runtime
spawns all the components that must execute on the \emph{deployment unit}.
%
...
%
Both experiments were released with a permissive open-source licence and archived for future reference on
...
we estimate the number of instructions for the pulverised components as follows:
...
%
We run 10 repetitions of each experiment with a different random seed;
...
\begin{figure}
    ...
    \caption{
        ...
        %
        $P_{cloud}$ and $P_{devices}$ are stacked to show the overall power consumption.
        %
        Different reconfiguration rules are indicated on the horizontal axis.
        %
        Runtime reconfiguration of the deployment opens more opportunities to find the
        cost, energy use, and \ac{qos} trade-off that best fits the current context.
    }
    \label{fig:results}
\end{figure}
%
...
%
$\Downarrow$ has the worst \ac{qos}
but also the lowest cloud cost and overall energy consumption.
%
Conversely, $\Uparrow$ improves the \ac{qos}, but costs and energy use grow significantly.
%
...
depending on the battery level allows us to find different trade-offs between these two extremes.
...
Languages have been proposed
 targeting specific deployment domains
...
%
The main aspects distinguishing \ourframework{} from other approaches are that it targets \emph{pulverised systems} (and not general partitionings) and that it uses internal \acp{dsl} embedded in Kotlin.

A related (external) \ac{dsl} is 
\emph{MuScADeL}~\cite{DBLP:conf/compsac/BoujbelRLTAL14/muscadel}:
it allows expressing deployment properties of
...
%
Component descriptions can include \emph{constraints}
(both basic software/hardware constraints and multi-scale criteria).
%
The specifications are used to generate \emph{probe} artefacts supporting data collection at the level of the deployment middleware.
%
...
%
Indeed, formal models can help specify reconfigurable architectures: examples include
...
\emph{connectors} (capturing the interaction between components' \emph{ports}),
...
%
These tools can support \emph{reasoning} about reconfigurations and \emph{verification}.

%\meta{TODO: say something about our positioning wrt verification of reconfigurations}

Several approaches use languages together with middlewares to specify and execute reconfiguration policies.
%
...
%
Similarly,
earlier approaches based on the specification of \emph{constraint-sets} include \emph{DELADAS}~\cite{Dearle2004deladas}
...
\emph{Ctrl-F}~\cite{DBLP:journals/jss/AlvaresRS17}
 ...
 and policies (constraints) for reconfiguring system components.
%
The idea of its \ac{dsl} to reconfiguration is to let designers specify behaviours that regulate the transitions between explicitly specified \emph{configurations}.
%
...
%
The approach is based on the \emph{GeneSIS} modelling language,
...
%
However,
these approaches, being based on constraint solving, typically suffer from scalability issues.

Finally, there is a plethora of approaches for automatic deployment or offloading across the edge-cloud continuum. One example is \emph{osmotic computing}~\cite{DBLP:journals/computer/VillariFDRJR19},
...
In this work,
we presented a practical \ac{dsl} and framework for the
...
%
The specialised \ac{dsl} introduced in this work
shows that this idea can be realised in practice.
%
Moreover, we extended the original idea of pulverisation with the possibility of specifying runtime re-configuration policies,
which can be leveraged to achieve better performance, cost, and resource usage trade-offs.
%
Finally, we provide a real-world demonstrator for the technology
and show via simulation that the approach can be beneficial.
...
%
A policy that requires to consider the state of multiple hosts would require resorting to the host language's primitive mechanisms and make explicit communications outside the framework,
thus breaking the abstraction.
%
This limitation can be addressed by extending the current reconfiguration
block of the \ac{dsl} so that queries can be performed on any reachable host of the system,
...
we intend to add support for additional network protocols to support a larger number of practical scenarios.
